...
const Cards = () => {
  ...
  const handleFilter = async () => {
    if (!conditionOption && !rarityOption) {
      ...
      const filteredProducts = await getProductsByCondition({
        conditionId: conditionOption,
        rarityId: rarityOption,
      });
      ...
      <div className="filter-container">
        ...
        <div className="filter-bar">
          <select
            ...
            onChange={(event) => setConditionOption(event.target.value)}
          >
          ...
          >
            ...
            {rarities.map((rare) => (
              <option key={rare.id} value={rare.id}>
                {rare.name}
              </option>
            ))}
          </select>
